...
class UsersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $userObj = $this->model->with('roles.permissions');

        if($name) {
            $userObj = $userObj->where('name', 'like', '%' . $name . '%');
        }

        $userObj = $userObj->paginate(env('rpp'));

        if ($request->has('limit')) {
            $userObj->appends('limit', $request->get('limit'));
        }

        return $this->response->paginator($userObj, new UserTransformer());
    }
    ...
    public function destroy(Request $request, $uuid)
    {
        ...
        foreach($user->customers as $customer) {
            foreach($customer->projects as $project) {
                Project::deleteProject($project->uuid);
            }
        }
        $user->delete();
